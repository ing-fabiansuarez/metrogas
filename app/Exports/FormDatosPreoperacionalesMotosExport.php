...
class FormDatosPreoperacionalesMotosExport implements FromQuery, ShouldAutoSize, WithHeadings, WithMapping, WithColumnFormatting
{
    ...
    public function query()
    {
        ...
        if ($this->num_solicitud != null) {
            $consulta->where('form_datos_preoperacionales_motos_models.id', $this->num_solicitud);
        }
        if ($this->start_date != null) {
            $consulta->whereDate('form_datos_preoperacionales_motos_models.created_at', '>=', $this->start_date);
        }
        if ($this->end_date != null) {
            $consulta->whereDate('form_datos_preoperacionales_motos_models.created_at', '<=', $this->end_date);
        }
        if ($this->placa_vehiculo != null) {
            $consulta->where('form_datos_preoperacionales_motos_models.placa_vehiculo', $this->placa_vehiculo);
        }
        if ($this->cedula != null) {
            $consulta->where('form_datos_preoperacionales_motos_models.cedula', $this->cedula);
        }
        return $consulta;
    }
