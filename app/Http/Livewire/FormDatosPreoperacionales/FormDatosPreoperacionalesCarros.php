...
class FormDatosPreoperacionalesCarros extends Component
{
    ...
    public function mount($solo_lectura = false, DatosPreoperacional $datosPreoperacional = new DatosPreoperacional(), FormDatosPreoperacionalesCarrosModel $formulario = new FormDatosPreoperacionalesCarrosModel(), $placa = null)
    {
        ...
        if ($placa) {
            if ($modeloPre = FormDatosPreoperacionalesCarrosModel::where('placa_vehiculo', $placa)->orderBy('id', 'desc')->first()) {
                $this->model = $modeloPre;
                $this->model->id = null;
            }
        }

        $this->model->placa_vehiculo = $placa;
        $this->model->tipo_vehiculo = ETipoVehiculo::CARRO->getId();
