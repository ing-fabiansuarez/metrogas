<div>
    <div class="card px-3 pt-4 pb-0 mt-0 mb-3">
        <div id="msform">
            <!-- progressbar -->
            <x-viatic-requests.progress-bar :stepsCompletes='5' />
            <br>

            <div class="alert alert-success" role="alert">
                <strong>Usuarios que pueden aprobar:</strong><br>
                @foreach ($viaticRequest->usersCanAproveTesoreria() as $user)
                    {{ $user->name . ' (' . $user->jobtitle->name . ')' }} <br>
                @endforeach
                @if (count($viaticRequest->usersCanAproveTesoreria()) <= 0)
            ...
            <fieldset>
                <div class="form-card">
                    <div class="row">
                        <div class="col-7">
                            <h2 class="fs-title">Aprobación Tesoreria</h2>
                        </div>
                        <div class="col-5">
        ...
        Livewire.on('beforeAproveViaticRequest', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.emitTo('process-viatic-request.aprove-tesoreria', 'aproveViaticRequest');
                }
            })
        });
